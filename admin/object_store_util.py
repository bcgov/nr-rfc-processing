...
class OStore:
    def __init__(self):
        ...
        self.historical_norms_path = "norm/modis/daily/{period}/{month}.{day}.tif"

    def get_10yr_tif(self, month, day, out_path):
        if not os.path.exists(out_path):
            ostore_path = self.historical_norms_path.format(
                period="10yr", month=month, day=day
            )
    ...
    def get_20yr_tif(self, month, day, out_path):
        if not os.path.exists(out_path):

            ostore_path = self.historical_norms_path.format(
                period="20yr", month=month, day=day
            )
